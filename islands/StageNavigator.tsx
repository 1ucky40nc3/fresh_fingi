...
const StageNavigatorIsland: FunctionComponent<StageNavigatorIslandProps> = (
  ...
  return (
    <div class="w-full bg-gray-800 text-white p-4 shadow-lg">
      <div class="max-w-3xl mx-auto flex justify-between items-center relative">
        {/* Connecting Lines */}
        <div class="absolute inset-x-0 top-1/2 h-1 bg-gray-600 -translate-y-1/2 z-0">
        </div>
        {/* Progress Line */}
        <div
          class={`absolute left-0 top-1/2 h-1 bg-blue-400 -translate-y-1/2 z-10 transition-all duration-500 ease-in-out`}
          style={{ width: `${(currentStage - 1) * 50}%` }}
        >
        </div>

        {stages.map((stage) => {
          // Determine if the stage is active, completed, or disabled
          const isActive = currentStage === stage.id;
          const isCompleted = completedStages >= stage.id;
          const isDisabled = stage.id > completedStages + 1; // Can only go to next logical stage
